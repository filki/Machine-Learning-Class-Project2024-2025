...
def calculate_stability(model, X, vectorizer, n_samples=5):
    ...
    for _ in range(n_samples):
        ...
        if isinstance(model, TruncatedSVD):
            ...
            model_sample = LatentDirichletAllocation(n_components=model.n_components,
                                                     random_state=42)
        else:  # NMF
...
def visualize_topics(model, X, method_name, save_path):
    ...
    # Próbkowanie danych dla UMAP, jeśli dataset jest zbyt duży
    max_samples = 50000  # UMAP radzi sobie z większymi zbiorami niż t-SNE
